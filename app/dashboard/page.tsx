...
import api, { DashboardStats, Appointment, authAPI } from '@/app/services/api';
      ...
      <main className="flex-1 p-8 overflow-y-auto">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-gradient-to-r from-[#b7b8f3] to-[#7b6ffb] rounded-2xl p-6 text-white shadow-lg flex flex-col relative overflow-hidden">
            <div className="flex items-center justify-between mb-2">
              <span className="flex items-center gap-2 font-medium">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><polygon points="12 2 15 8.5 22 9.3 17 14.1 18.2 21 12 17.8 5.8 21 7 14.1 2 9.3 9 8.5 12 2"/><path d="M12 8v4l3 3"/></svg>
            ...
            <table className="w-full text-left">
              ...
              <tbody>
                {recentAppointments.map((appointment) => (
                  <tr key={appointment._id} className="border-t border-gray-100  text-black hover:bg-gray-50">
                    <td className="py-3 px-2">{appointment.patient?.name || 'Unknown'}</td>
                    <td className="py-3 px-2">{appointment.doctor?.name || 'Unknown'}</td>
                    <td className="py-3 px-2">{new Date(appointment.date).toLocaleDateString()}</td>
                    <td className="py-3 px-2">{appointment.time}</td>
                    <td className="py-3 px-2">
                      <span className={`px-3 py-1 rounded-full text-xs font-semibold ${statusColors[appointment.status as keyof typeof statusColors] || 'bg-gray-100 text-gray-700'}`}>{appointment.status}</span>
                    </td>
                  </tr>
                ))}
              </tbody>
